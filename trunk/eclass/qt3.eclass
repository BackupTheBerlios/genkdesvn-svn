# Copyright 2005 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# $Header: /var/cvsroot/gentoo-x86/eclass/qt3.eclass,v 1.7 2005/07/25 14:53:25 caleb Exp $
#
# Author Caleb Tennis <caleb@gentoo.org>
#
# This eclass is simple.  Inherit it, and in your depend, do something like this:
#
# DEPEND="$(qt_min_version 3.1)"
#
# and it handles the rest for you
#
# Caveats:
#
# Currently, the ebuild assumes that a minimum version of Qt3 is NOT satisfied by Qt4

inherit versionator

QTPKG="x11-libs/qt-"
QT3MAJORVERSIONS="3.3 7 3.2 3.1 3.0"
QT3VERSIONS="3.3.4-r8 3.3.4-r7 3.3.4-r6 7 3.3.4-r5 3.3.4-r4 3.3.4-r3 3.3.4-r2 3.3.4-r1 3.3.4
3.3.3-r3 3.3.3-r2 3.3.3-r1 3.3.3 3.3.2 3.3.1-r2 3.3.1-r1 3.3.1 3.3.0-r1 3.3.0
3.2.3-r1 3.2.3 3.2.2-r1 3.2.2 3.2.1-r2 3.2.1-r1 3.2.1 3.2.0 3.1.2-r4 3.1.2-r3
3.1.2-r2 3.1.2-r1 3.1.2 3.1.1-r2 3.1.1-r1 3.1.1 3.1.0-r3 3.1.0-r2 3.1.0-r1 3.1.0"

if [[ -z $QTDIR ]]; then
	[ -d ${ROOT}/var/db/pkg/x11-libs/qt-7* ] && QTDIR="/usr/qt/devel" || QTDIR="/usr/qt/3"
fi

addwrite "$QTDIR/etc/settings"
addpredict "$QTDIR/etc/settings"

qt_min_version() {
	echo "|| ("
	qt_min_version_list "$@"
	echo ")"
}

qt_min_version_list() {
	local MINVER=$1
	local VERSIONS=""

	case $MINVER in
		3|3.0|3.0.0) VERSIONS="=${QTPKG}3* =${QTPKG}7*";;
		3.1|3.1.0|3.2|3.2.0|3.3|3.3.0) 
			for x in $QT3MAJORVERSIONS; do
				if $(version_is_at_least $MINVER $x); then
					VERSIONS="${VERSIONS} >=${QTPKG}${x}*"
				fi
			done
			;;
		3*)
			for x in $QT3VERSIONS; do
				if $(version_is_at_least $MINVER $x); then
					VERSIONS="${VERSIONS} >=${QTPKG}${x}"
				fi
			done
			;;
		*) VERSIONS="=${QTPKG}3* =${QTPKG}7*";;
	esac

	echo "$VERSIONS"
}
