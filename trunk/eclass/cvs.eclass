...
process_fetch() {
	...
	if [ -n "$cmdupdate1" ]; then
		einfo "Running $cmdupdate1"
		eval $cmdupdate1 || die "cvs update command failed"
	fi

	# Process local checkout
	if [ -n "$cmdcheckout1" ]; then
		einfo "Running $cmdcheckout1"
		eval $cmdcheckout1 || die "cvs checkout command failed"
	fi

	# Process recursive update
	if [ -n "$cmdupdate" ]; then
		einfo "Running $cmdupdate"
		eval $cmdupdate || die "cvs update command failed"
	fi

	# Process recursive checkout
	if [ -n "$cmdcheckout" ]; then
...
# Should be used for recursive checkouts ONLY
# Takes out paths that are subpaths of another path in the list of paths passed
# Try to pronounce the last sentence...
# CVS has trouble otherwise: move away .... it is in the way
# The reason this should not be applicable to the local checkouts is that
# checking out the upper path doesn't guarantee checkout of the lower path
prune_nested_paths() {

	local listref=\$"$1"
	local list=`eval "expr \"$listref\" "`
	local result=""
	...
	do
		...
		do
			...
			[ "${item:0:${#other}}" == "$other" ] && [ "${item:${#other}:1}" == "/" ] && nested=true
...
cvs_fetch() {
	...
	# Supporting multiple-item fetch, ECVS_LOCAL and ECVS_LOCALNAME are unused in that context
	# When using archaic single checkout
	cmdupdatetemplate="${run} ${ECVS_CVS_COMMAND} -d \"${cvsroot_nopass}\" update ${ECVS_UP_OPTS}"
	cmdcheckouttemplate="${run} ${ECVS_CVS_COMMAND} -d \"${cvsroot_nopass}\" checkout ${ECVS_CO_OPTS}"

	# If single-item fetch decide whether to update or checkout
	if [ -z "$ESCM_DEEPITEMS" ] && [ -z "$ESCM_SHALLOWITEMS" ]; then
	
		if [ {$mode} == "update" ]; then
			cmdupdate="$cmdupdatetemplate ${ECVS_LOCALNAME}"
		else
			cmdcheckout="$cmdcheckouttemplate ${ECVS_MODULE}"
		fi

	# If multiple-item fetch, decide which items to update or checkout and which
	# to do so recursively and which to do non-recursively
	else

		# A (not unreasonable) assumption here is that there are *always* deep items to checkout
		# Sort non-recursive items for checkout or update
		if [ -n "$ESCM_SHALLOWITEMS" ]; then
			
			local updateitems1=""
			local checkoutitems1=""

			for item in $ESCM_SHALLOWITEMS
			do
			
				if [ -e "${ECVS_TOP_DIR}/$item" ]; then
					updateitems1="$updateitems1 $item"
				else
					checkoutitems1="$checkoutitems1 $item"
				fi

			done
			[ "$updateitems1" != "" ] && cmdupdate1="$cmdupdatetemplate -l $updateitems1" 
			[ "$checkoutitems1" != "" ] && cmdcheckout1="$cmdcheckouttemplate -l $checkoutitems1" 
		fi

		# Sort recursive items for checkout or update
		if [ -n "$ESCM_DEEPITEMS" ]; then
			...
			[ "$updateitems" != "" ] && prune_nested_paths updateitems && cmdupdate="$cmdupdatetemplate -R $updateitems" 
			[ "$checkoutitems" != "" ] && prune_nested_paths checkoutitems && cmdcheckout="$cmdcheckouttemplate -R $checkoutitems" 
	...
	if [ "${ECVS_AUTH}" == "pserver" ]; then
		...
		# Finally process fetch (update/checkout)
		process_fetch
...
cvs_src_unpack() {
	...
	if [ -n "$ESCM_DEEPITEMS" ] || [ -n "$ESCM_SHALLOWITEMS" ]; then
		storedir_to_workdir
	else
